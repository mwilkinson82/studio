...
// Import SheetClose directly from Radix as we are using SheetPrimitive.Content
import { SheetPortal, SheetOverlay, SheetTitle, SheetDescription } from "@/components/ui/sheet";
import * as SheetPrimitive from "@radix-ui/react-dialog"; // Keep this for Root, Content, Close
...
>(
  (
    ...
    const contextValue = React.useMemo<SidebarContext>(() => ({ state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar }), [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar])
    return (
...
const panelVariants = {
  hidden: { height: 0, opacity: 0, transition: { duration: 0.7, ease: [0.4, 0, 0.2, 1] } }, 
  visible: { height: "100vh", opacity: 1, transition: { duration: 0.5, ease: [0.4, 0, 0.2, 1] } }
};
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
        ...
        <AnimatePresence mode="wait">
          {openMobile && (
            <SheetPortal forceMount>
              ...
              {/* Use SheetPrimitive.Content and nest motion.div for animation if Radix handles accessibility better */}
              <SheetPrimitive.Content 
                className="fixed inset-0 z-50 p-0 border-none" /* Base positioning, no visual style */
                asChild
              >
                <motion.div
                  className={cn(
                    "h-full w-full flex flex-col bg-white text-foreground shadow-lg", // Actual visual panel styles
                    "overflow-hidden" // Crucial for hiding content that overflows during animation
                  )}
                  initial="hidden"
                  animate="visible"
                  exit="hidden"
                ...
                >
                  {/* These are for accessibility, Radix wires them up. */}
                  <SheetTitle className="sr-only">Main Menu</SheetTitle>
                  <SheetDescription className="sr-only">Application navigation and options.</SheetDescription>
  ...
  return (
    <div ref={ref} className={cn("group peer hidden md:block text-sidebar-foreground", className)} data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side}>
...
const SidebarMenuButton = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button"> & { asChild?: boolean, isActive?: boolean, tooltip?: string | React.ComponentProps<typeof TooltipContent> } & VariantProps<typeof sidebarMenuButtonVariants>>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
  const Comp = asChild ? Slot : "button"; const { isMobile, state } = useSidebar();
